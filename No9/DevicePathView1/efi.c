#include "efi.h"
#include "common.h"

struct EFI_SYSTEM_TABLE *ST;
struct EFI_GRAPHICS_OUTPUT_PROTOCOL *GOP;   /* GUI */
struct EFI_SIMPLE_POINTER_PROTOCOL *SPP;    /* マウス */
struct EFI_SIMPLE_FILE_SYSTEM_PROTOCOL *SFSP; /* ファイルシステム */
struct EFI_SIMPLE_TEXT_INPUT_EX_PROTOCOL *STIEP; // 高機能なキー入力

/* ウォッチドッグタイマとプロトコルの先頭アドレスの取得 */
void efi_init(struct EFI_SYSTEM_TABLE *SystemTable)
{
    /* EFI_GRAPHICS_OUTPUT_PROTOCOL */
    struct EFI_GUID gop_guid = {0x9042a9de, 0x23dc, 0x4a38,     // ここに  \   をつけるには注意（てかなんでつけたんだ...?）
                                {0x96, 0xfb, 0x7a, 0xde,
                                 0xd0, 0x80, 0x51, 0x6a}};
    
    /* EFI_SIMPLE_POINTER_PROTOCOL（仕様書では0x0が0x00） */
    struct EFI_GUID spp_guid = {0x31878c87, 0xb75, 0x11d5,
                                {0x9a, 0x4f, 0x0, 0x90,
                                0x27, 0x3f, 0xc1, 0x4d}};

    /* EFI_SIMPLE_FILE_SYSTEM_PROTOCOL */
    struct EFI_GUID sfsp_guid = {0x0964e5b22, 0x6459, 0x11d2,
                                {0x8e, 0x39, 0x00, 0xa0,
                                 0xc9, 0x69, 0x72, 0x3b}};
    
    // EFI_SIMPLE_TEXT_INPUT_EX_PROTOCOl
    struct EFI_GUID stiep_guid = {0xdd9e7534, 0x7762, 0x4698,
                                 {0x8c, 0x14, 0xf5, 0x85,
                                  0x17, 0xa6, 0x25, 0xaa}};

    ST = SystemTable;
    ST->BootServices->SetWatchdogTimer(0, 0, 0, NULL);
    ST->BootServices->LocateProtocol(&gop_guid, NULL, (void **)&GOP);
    ST->BootServices->LocateProtocol(&spp_guid, NULL, (void **)&SPP);
    ST->BootServices->LocateProtocol(&sfsp_guid, NULL, (void **)&SFSP);
    ST->BootServices->LocateProtocol(&stiep_guid, NULL, (void **)&STIEP);
}
